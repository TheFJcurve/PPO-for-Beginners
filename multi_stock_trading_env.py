...
class PersonalMultiStockTradingEnv(gym.Env):
    ...
    def process_data(self):
        ...
        for i in range(self.assets):
            ...
            if self.scalers[i]:
                ...
                signal_features_i = current_scaler.transform(
                    df.loc[:, self.tech_indicators])[start:end]
            else:
                current_scaler = StandardScaler()
                signal_features_i = current_scaler.fit_transform(
                    df.loc[:, self.tech_indicators])[start:end]
        ...
        if self.representative:
            self.representative = self.price_df.loc[:, self.representative].to_numpy()[
                start:end]
        else:
            self.representative = self.price_df.loc[:, 'SENSEX'].to_numpy()[
                start:end]
    ...
    def _update_profit(self, ):
        self._total_profit = (self.PortfolioValue +
                              self.reserve)/self.initial_amount
    ...
    def step(self, actions):
        ...
        norm_margin_pos = (abs_portfolio_dist /
                           sum(abs_portfolio_dist))*self.margin
